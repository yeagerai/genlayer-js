...
import {alt, apply, kleft, kright, list, seq, tok, opt} from "typescript-parsec";
...
TERM.setPattern(
  alt(
    ...
    apply(tok(TokenKind.Addr), v => new Address(parseHex(v.text.substring(5)))),
  ),
);

ARR.setPattern(parseArray(tok(TokenKind.LSpar), EXPR, tok(TokenKind.Comma), tok(TokenKind.RSpar)));
...
MAP.setPattern(
  apply(
    parseArray(
      tok(TokenKind.LCpar),
      apply(seq(ARR_ID, tok(TokenKind.Colon), EXPR), (x): [string, CalldataEncodable] => [x[0], x[2]]),
      tok(TokenKind.Comma),
      tok(TokenKind.RCpar),
    ),
    v => new Map(v),
...
export function parse(s: string): CalldataEncodable {
  const quotedParam = typeof s === "string" ? `"${s}"` : s;
  return expectSingleResult(expectEOF(EXPR.parse(lexer.parse(quotedParam))));
}
